...
-- 1. 문자처리함수
-- LENGTH, LENGTHB(길이구함), SUBSTR(문자자름), INSTR(위치 구함)
-- LPAD, RPAD
...
-- 사원명에서 성만 중복없이 사전순으로 출력하시오
SELECT DISTINCT SUBSTR(EMP_NAME, 1, 1)"EMP_NAME"
FROM EMPLOYEE
ORDER BY 1 ASC;
-- 1. 컬럼명과 똑같이 적는다 (EMP_NAME, 1, 1)
-- 2. 별칭을 지정해서 적는다 EMP_NAME
-- 3. 컬럼의 첫번째 자리니까 1을 적는다
-- 중복없이 : DISTINCT
...
-- 3. 날짜처리함수
-- EMPLOYEE 테이블에서 이름, 근무일수를 출력해보시오
...
-- EMPLOYEE 테이블에서 사원의 이름, 입사일, 입사후 3개월이 된 날짜를 조회하시오
...
-- EMPLOYEE 테이블에서 사원의 이름, 입사일, 입사월의 마지막날을 조회하시오
SELECT EMP_NAME, HIRE_DATE, LAST_DAY(HIRE_DATE)
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 사원 이름, 입사 연도, 입사 월, 입사 일을 조회하시오
...
/*
     ...
     (단, 1일 3끼를 먹는다고 한다.)
*/
SELECT ADD_MONTH(SYSDATE, 18) "제대일자", (ADD_MONTH(SYSDATE, 18) - SYSDATE)*3 "짬밥수"
FROM DUAL;
...
-- 문자 처리 함수
, EXTRACT(YEAR FROM TO_DATE(SBSTR(EMP_NO,1,2), 'RR'))
||'년 '|| EXTRACT(MONTH FROM TO_DATE(SBSTR(EMP_NO,3,2), 'MM'))
||'월 '|| EXTRACT(DAY FROM TO_DATE(SBSTR(EMP_NO,5,2), 'DD'))
||'일' AS 생년월일2
-- 날짜 처리 함수
, EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(SBSTR(EMP_NO,1,2), 'RR')) AS "나이(만)"
-- 날짜 처리 함수
...
SELECT EMP_NAME AS 이름, EMP_NO AS 주민번호, SARARY AS 급여, HIRE_DATE AS 입사일
FROM EMPLOYEE
...
--(퇴사 여부 : ENT_YN)
SELECT EMP_NAME AS 이름, DEPT_CODE AS 부서코드, HIRE_DATE AS 입사일
, (ENT_DATE-HIREDATE) AS 근무기간,  ENT_DATE AS 퇴사일
FROM EMPLOYEE
-- WHERE ENT_YN != 'N';
WHERE ENT_YP = 'Y';
...
SELECT EMP_NAME AS 이름, EMP_NO AS 주민번호, EMAIL AS 이메일,PHONE AS 폰번호, SARARY AS 급여
FROM EMPLOYEE
...
SELECT EMP_NAME AS 이름, ((SYSDATE-HIRE_DATE)/1000)*0.1*SALARY AS "특별 보너스"
FROM EMPLOYEE
